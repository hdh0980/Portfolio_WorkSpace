...
- **핵심 정리:** 같은 회로를 노달해석과 메시해석으로 풀면 대응하는 노드전압과 가지전류가 같아야 하며, KCL·KVL 잔차는 해의 일치를 판단하는 기준이 된다.
- **확인 근거:** 노달 예제는 $V_1=6$ V, $V_2=3$ V에서 각 노드의 유입·유출 전류가 일치하고, 메시 예제는 $i_1=4.364$ mA, $i_2=1.091$ mA와 공유저항 전류 3.273 mA로 두 루프의 KVL을 반올림 오차 안에서 만족한다.
- **다음 탐구:** 하나의 평면회로를 노달·메시 두 방식으로 각각 풀어 공통 가지전류와 방정식 잔차를 직접 비교한다.

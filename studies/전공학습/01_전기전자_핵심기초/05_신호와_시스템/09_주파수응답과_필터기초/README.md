...
- **EV OBC 전류·전압 센서 AAF**: 센서 아날로그 출력 → RC(또는 Sallen-Key 능동 2차) 필터 → ADC. 설계 기준: 통과대역($\leq 2\,\text{kHz}$) 위상 지연 최소화, 폴딩 주파수($31.25\,\text{kHz}$)에서 $-40\,\text{dB}$ 이상. Bessel을 쓰면 파형 왜곡이 최소지만 감쇠가 부족하고, Butterworth 2차로 타협.
- **군지연(group delay)**: 전류 제어기 루프에서 필터 위상 지연이 루프 위상 마진을 줄인다. $\tau_g = -d\angle H/d\omega$를 계산하여 루프 대역($2\,\text{kHz}$)에서의 지연을 확인.
- **디지털 필터 구현**: 아날로그 설계 완료 후 Bilinear(Tustin) 변환으로 $H(z)$를 도출. $\omega_c$가 $f_s$에 비해 크면 사전 주파수 왜곡(pre-warping) 필요:

  $$\omega_{\text{prewarp}} = \frac{2}{T_s}\tan\!\left(\frac{\omega_c T_s}{2}\right)$$

- **Notch 필터**: 특정 고조파(예: 125 kHz = $2f_{\text{sw}}$) 제거에 사용. 이산 biquad 구조로 구현.

---

## 9. 자가 점검

1. $H(j\omega) = \frac{1}{1+j\omega/100}$에서 $\omega=1000\,\text{rad/s}$일 때 크기[dB]와 위상을 구하라.
2. Bessel 필터를 ADC AAF 대신 Butterworth보다 선호하는 이유는? 단점도 설명하라.
3. 이산 LPF $H(e^{j\omega})$에서 나이퀴스트 주파수($\omega=\pi$)가 물리적으로 의미하는 것은?

<details><summary>정답</summary>

1. $\omega/\omega_c = 10$ → $|H| = 1/\sqrt{101} \approx 0.0995$, $20\log(0.0995) \approx -20.0\,\text{dB}$. 위상 = $-\arctan(10) \approx -84.3°$.
2. Bessel: 군지연이 일정하여 파형 형상 보존 → 전류 파형 왜곡 최소. 단점: 전이대역 감쇠가 완만해 같은 차수에서 에일리어싱 차단 능력이 약하다. 고차로 보완 필요.
3. $\omega = \pi$ [rad/sample]는 샘플링 주파수의 절반($f_s/2$), 즉 나이퀴스트 주파수. 이보다 높은 이산 주파수는 아날로그 신호에서 나이퀴스트 주파수 위와 등가이므로 에일리어스로 나타난다.

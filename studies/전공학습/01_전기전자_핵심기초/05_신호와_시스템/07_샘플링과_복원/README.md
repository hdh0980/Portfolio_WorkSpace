...
- **핵심 정리:** 대역제한 신호를 복원하려면 $f_s\geq2f_M$를 만족해야 하고, 이를 넘는 입력 성분은 샘플링 주파수의 정수배만큼 접혀 기저대역의 다른 주파수로 나타난다.
- **확인 근거:** 최고 주파수 $1.2\ \text{kHz}$인 신호의 나이퀴스트 레이트는 $2.4\ \text{kHz}$이고, $f_s=8\ \text{kHz}$에서 $9.5\ \text{kHz}$ 성분은 $|9.5-8|=1.5\ \text{kHz}$로 에일리어싱된다.
- **다음 탐구:** 같은 $1.2\ \text{kHz}$ 대역제한 신호를 $2.4$, $4$, $8\ \text{kHz}$로 샘플링할 때 스펙트럼 복제본의 위치와 복원 필터의 전이대역 여유를 각각 계산할 것이다.

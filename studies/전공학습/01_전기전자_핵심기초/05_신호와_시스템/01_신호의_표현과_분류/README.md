...
- **핵심 정리:** 신호를 연속·이산, 주기·비주기, 에너지·전력, 우·기함수로 나누고 각 분류의 정의를 식으로 판정하는 흐름을 정리했다.
- **확인 근거:** $x(t)=3e^{-2t}u(t)$의 에너지는 $2.25\ \text{J}$로 유한하고, $x[n]=\cos(3\pi n/7)$은 $3N=14k$를 만족하는 최소 정수 $N_0=14$를 기본 주기로 갖는다.
- **다음 탐구:** 주파수가 다른 두 이산 정현파의 합을 정하고 각 성분 주기의 최소공배수로 전체 신호의 주기 존재 여부와 기본 주기를 판정할 것이다.
